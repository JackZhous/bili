package com.jack.zhou.bili.ui.view;
...
public class HotSearchView extends ViewGroup {
    ...
    private static final int expand_space = 100;                                                     //默认增大布局
    ...
    private int moveUpDistance;                                               //组件向上滑动和向下滑动的最大距离
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        parent_bottom = b;
        parent_width = r;
        JLog.default_print("changed " + changed + " l " + l + " t " + t + " r " + r + " b " + b);
    ...
    private void onLayoutChildView(int start_index, int end_index, int start_x, int start_y, int space){
        ...
        for(i = start_index; i < end_index; i++){
            ...
            if(second_enter < 1){                                                                   //分摊只需要在前两次进行分摊，后续不在分摊；因为后续布局都已经完成，再次分摊会照成重新获取padding值，该值会累加的
        ...
        if(i == childCount){                                                                        //最后一行时，计算父组件最大值和child最下面的Y值，计算差值作为向上滑动的最大距离
            moveUpDistance = endY - parent_bottom + default_space + 200;
        }
